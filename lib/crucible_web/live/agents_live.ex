defmodule CrucibleWeb.AgentsLive do
  ...
  def render(assigns) do
      ...
      <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-4">
          <div>
            ...
            <p class="font-label text-xs text-[#00eefc]/60 uppercase tracking-widest">
              Entity Definitions & Configuration Registry
            </p>
          </div>
        ...
        <div :if={@agents == []} class="bg-surface-container-low p-12 hud-border text-center">
          ...
          <p class="font-label text-[10px] text-[#adaaaa]/60 uppercase tracking-widest">
            NO_AGENT_DEFINITIONS_FOUND
          </p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div
            :if={@agents != []}
            class={"#{if @selected, do: "lg:col-span-8", else: "lg:col-span-12"} space-y-4"}
          >
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                <div class="flex justify-between items-start mb-3">
                  <div>
                    <div class={"font-label text-xs font-bold #{if @selected == agent.name, do: "text-[#00eefc]", else: "text-[#ffa44c]"}"}>
                      {humanize_name(agent.name)}
                    </div>
                    <div class="font-label text-[9px] text-[#ffa44c]/60 uppercase">
                ...
                <div class="flex flex-wrap gap-1.5">
                  ...
                  <span
                    :if={length(agent.capabilities) > 3}
                    class="text-[8px] font-label px-1.5 py-0.5 text-[#adaaaa]/40"
                  >
              ...
              <div :if={agent}>
                <div class="p-6 border-b border-[#494847]/20 bg-gradient-to-br from-[#ffa44c]/5 to-transparent">
                  <div class="flex items-center justify-between mb-4">
                    <h3 class="font-headline text-xl font-bold text-[#ffa44c] uppercase tracking-widest">
                      {humanize_name(agent.name)}
                    </h3>
                    <button
                      phx-click="select_agent"
                      phx-value-name={agent.name}
                      class="text-[#ffa44c]/40 hover:text-[#ffa44c]"
                    >
                      <span class="material-symbols-outlined text-sm">close</span>
                    </button>
                  </div>

                  <p :if={agent.description} class="text-xs text-[#adaaaa]/80 mb-4">
                    {agent.description}
                  </p>
                  ...
                  <div class="space-y-4">
                    ...
                    <div>
                      <div class="flex items-center gap-2 mb-2">
                        ...
                        <span class="font-label text-[9px] text-[#ffa44c] uppercase font-bold tracking-widest">
                          MODEL
                        </span>
                      </div>
                    ...
                    <div>
                      ...
                      <div class="flex flex-wrap gap-1">
                        <span
                          ...
                          class="bg-surface-container p-2 border border-[#494847]/10 text-[10px] font-label text-[#adaaaa]/80"
                        >
                          {cap}
                        </span>
                        <span
                          :if={agent.capabilities == []}
                          class="text-[10px] font-label text-[#adaaaa]/40 italic"
                        >
                          NONE_SPECIFIED
                        </span>
                      </div>
    ...
    <!-- Denied Tools -->
                    <div>
                      <div class="flex items-center gap-2 mb-2">
                        ...
                        <span class="font-label text-[9px] text-[#ff725e] uppercase font-bold tracking-widest">
                          DENIED_TOOLS ({length(agent.denied_tools)})
                        </span>
                      </div>
                      <div class="flex flex-wrap gap-1">
                        <span
                          :for={tool <- agent.denied_tools}
                          class="text-[10px] font-label px-1.5 py-0.5 bg-[#ff725e]/10 text-[#ff725e] border border-[#ff725e]/20"
                        >
                          {tool}
                        </span>
                        <span
                          :if={agent.denied_tools == []}
                          class="text-[10px] font-label text-[#adaaaa]/40"
                        >
                          NONE
                        </span>
                      </div>
    ...
    <!-- System prompt -->
                <div :if={agent.system_prompt} class="p-4 bg-black/40 flex-1">
                  <div class="font-label text-[8px] text-[#00eefc]/40 uppercase mb-3 tracking-tighter">
    ...
    <!-- Voice Sessions -->
        <div class="bg-surface-container-low hud-border p-4">
          <div class="flex items-center gap-2 mb-3">
            ...
            <span
              :if={@voice_sessions != []}
              class="ml-auto px-2 py-0.5 bg-[#00eefc]/10 border border-[#00eefc]/30 font-label text-[9px] text-[#00eefc]"
            >
          ...
          <div :if={@voice_sessions != []} class="space-y-2">
            ...
            >
              <div class="flex items-center gap-2">
                ...
                <span class="font-label text-[9px] text-[#adaaaa]">
                  {String.slice(to_string(vs.session_id), 0, 16)}…
                </span>
              ...
              <div
                :if={vs[:transcript]}
                class="font-label text-[9px] text-[#adaaaa]/60 italic truncate"
              >
  ...
  defp nerv_role_badge(name) do
    cond do
      String.contains?(name, "coder") ->
        "bg-[#00eefc]/10 text-[#00eefc] border border-[#00eefc]/20"

      String.contains?(name, "reviewer") or String.contains?(name, "review") ->
        "bg-[#ffa44c]/10 text-[#ffa44c] border border-[#ffa44c]/20"

      String.contains?(name, "architect") ->
        "bg-[#00eefc]/10 text-[#00eefc] border border-[#00eefc]/20"

      String.contains?(name, "researcher") or String.contains?(name, "research") ->
        "bg-[#ff725e]/10 text-[#ff725e] border border-[#ff725e]/20"

      String.contains?(name, "analyst") ->
        "bg-[#ffa44c]/10 text-[#ffa44c] border border-[#ffa44c]/20"

      true ->
        "bg-[#494847]/20 text-[#adaaaa] border border-[#494847]/30"
    end
  end
  ...
  defp voice_status_badge("listening"),
    do: "bg-[#00eefc]/10 text-[#00eefc] border border-[#00eefc]/20"

  defp voice_status_badge("processing"),
    do: "bg-[#ffa44c]/10 text-[#ffa44c] border border-[#ffa44c]/20"
