defmodule CrucibleWeb.WorkspacesLive do
  ...
  def handle_event("toggle_form", _params, socket) do
    {:noreply, assign(socket, show_form: !socket.assigns.show_form, editing: nil, form: new_form())}
  end
  ...
  def handle_event("edit", %{"id" => id}, socket) do
    case WorkspaceProfiles.get_workspace(id) do
      %WorkspaceProfile{} = ws ->
        form =
          %{"name" => ws.name, "slug" => ws.slug, "repo_path" => ws.repo_path,
            "tech_context" => ws.tech_context, "default_workflow" => ws.default_workflow,
      ...
      %WorkspaceProfile{} = ws ->
        case WorkspaceProfiles.update_workspace(ws, params) do
          {:ok, _ws} ->
            {:noreply,
             socket
             |> assign(workspaces: WorkspaceProfiles.list_workspaces(), show_form: false, editing: nil, form: new_form())
      ...
      nil ->
        case WorkspaceProfiles.create_workspace(params) do
          {:ok, _ws} ->
            {:noreply,
             socket
             |> assign(workspaces: WorkspaceProfiles.list_workspaces(), show_form: false, form: new_form())
          ...
          <div :if={@show_form} class="mb-6 p-4 border border-[#ffa44c]/20 bg-[#111111]">
            ...
            <form phx-submit="save" class="space-y-3">
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="block text-[9px] text-[#b0b0b0] font-label uppercase tracking-widest mb-1">NAME</label>
                ...
                <div>
                  <label class="block text-[9px] text-[#b0b0b0] font-label uppercase tracking-widest mb-1">SLUG</label>
                  <input type="text" name="slug" value={@form[:slug].value} required pattern="[a-z0-9_-]+"
                    class="w-full bg-[#1a1a1a] border border-[#494847]/30 text-[#e0e0e0] text-xs px-3 py-2 focus:border-[#ffa44c]/50 focus:outline-none" />
                </div>
              </div>
              <div>
                <label class="block text-[9px] text-[#b0b0b0] font-label uppercase tracking-widest mb-1">REPO_PATH</label>
                <input type="text" name="repo_path" value={@form[:repo_path].value} required
                  class="w-full bg-[#1a1a1a] border border-[#494847]/30 text-[#e0e0e0] text-xs px-3 py-2 focus:border-[#ffa44c]/50 focus:outline-none"
                  placeholder="/path/to/your/repo" />
              </div>
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-[9px] text-[#b0b0b0] font-label uppercase tracking-widest mb-1">DEFAULT_WORKFLOW</label>
                  <input type="text" name="default_workflow" value={@form[:default_workflow].value || "coding-sprint"}
                    class="w-full bg-[#1a1a1a] border border-[#494847]/30 text-[#e0e0e0] text-xs px-3 py-2 focus:border-[#ffa44c]/50 focus:outline-none" />
                </div>
                <div>
                  <label class="block text-[9px] text-[#b0b0b0] font-label uppercase tracking-widest mb-1">DEFAULT_BRANCH</label>
                  <input type="text" name="default_branch" value={@form[:default_branch].value || "main"}
                    class="w-full bg-[#1a1a1a] border border-[#494847]/30 text-[#e0e0e0] text-xs px-3 py-2 focus:border-[#ffa44c]/50 focus:outline-none"
                    placeholder="main" />
                </div>
              </div>
              <div>
                <label class="block text-[9px] text-[#b0b0b0] font-label uppercase tracking-widest mb-1">TECH_CONTEXT</label>
                <textarea name="tech_context" rows="6"
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  <div class="flex items-center gap-3 mb-1">
                    ...
                    <span class="text-[9px] font-label text-[#00eefc] bg-[#00eefc]/10 px-2 py-0.5 tracking-wider"><%= ws.slug %></span>
                    <span class="text-[9px] font-label text-[#b0b0b0] tracking-wider"><%= ws.default_workflow %></span>
                    <span :if={ws.default_branch && ws.default_branch != "main"} class="text-[9px] font-label text-[#ff725e] bg-[#ff725e]/10 px-2 py-0.5 tracking-wider"><%= ws.default_branch %></span>
                  </div>
                  <div class="text-[10px] text-[#adaaaa] font-mono"><%= ws.repo_path %></div>
                  <div :if={ws.tech_context != ""} class="text-[10px] text-[#808080] mt-2 line-clamp-2">
                    <%= String.slice(ws.tech_context, 0, 200) %><%= if String.length(ws.tech_context) > 200, do: "..." %>
                  </div>
                </div>
                <div class="flex gap-2 ml-4">
                  <button phx-click="edit" phx-value-id={ws.id}
                  ...
                  <button phx-click="delete" phx-value-id={ws.id}
  ...
  defp new_form do
    %{"name" => "", "slug" => "", "repo_path" => "", "tech_context" => "", "default_workflow" => "coding-sprint", "default_branch" => "main"}
    |> to_form()
  end
end
