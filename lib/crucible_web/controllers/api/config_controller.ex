defmodule CrucibleWeb.Api.ConfigController do
  ...
  def env(conn, _params) do
    ...
    vars =
      Enum.map(keys, fn key ->
        val = System.get_env(key) || Secrets.get(key)
  ...
  def budget_config(conn, _params) do
    limits = Application.get_env(:crucible, :orchestrator, [])

    json(conn, %{
      dailyLimit: Keyword.get(limits, :daily_budget_usd, 100.0),
      agentLimit: Keyword.get(limits, :agent_budget_usd, 10.0),
      taskLimit: Keyword.get(limits, :task_budget_usd, 50.0)
    })
  end
  ...
  def update_budget(conn, params) do
    ...
    with {:ok, changes} <- validate_budget_params(params) do
      current = Application.get_env(:crucible, :orchestrator, [])
      previous = Keyword.take(current, [:daily_budget_usd, :agent_budget_usd, :task_budget_usd])

      updated =
        current
        |> maybe_put(changes, "dailyLimit", :daily_budget_usd)
        |> maybe_put(changes, "agentLimit", :agent_budget_usd)
        |> maybe_put(changes, "taskLimit", :task_budget_usd)

      Application.put_env(:crucible, :orchestrator, updated)

      # Audit trail
      Logger.info(
        "Budget config updated: #{inspect(previous)} -> #{inspect(Keyword.take(updated, [:daily_budget_usd, :agent_budget_usd, :task_budget_usd]))}"
      )
  ...
  defp redact(key, val) do
    if Secrets.sensitive_key?(key) do
      "***"
    else
      val
    end
