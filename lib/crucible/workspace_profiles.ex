defmodule Crucible.WorkspaceProfiles do
  ...
  def create_workspace(attrs) do
    ...
    with {:ok, ws} <- result do
      AuditLog.log("workspace", ws.id, "created", %{name: ws.name, slug: ws.slug},
        actor: "context:WorkspaceProfiles"
      )
  ...
  @spec update_workspace(WorkspaceProfile.t(), map()) ::
          {:ok, WorkspaceProfile.t()} | {:error, Ecto.Changeset.t()}
  def update_workspace(%WorkspaceProfile{} = ws, attrs) do
    result = ws |> WorkspaceProfile.changeset(attrs) |> Repo.update()

    with {:ok, updated} <- result do
      AuditLog.log("workspace", ws.id, "updated", %{fields: Map.keys(attrs)},
        actor: "context:WorkspaceProfiles"
      )
  ...
  @spec delete_workspace(WorkspaceProfile.t()) ::
          {:ok, WorkspaceProfile.t()} | {:error, Ecto.Changeset.t()}
  def delete_workspace(%WorkspaceProfile{} = ws) do
    result = Repo.delete(ws)

    with {:ok, deleted} <- result do
      AuditLog.log("workspace", ws.id, "deleted", %{name: ws.name},
        actor: "context:WorkspaceProfiles"
      )
